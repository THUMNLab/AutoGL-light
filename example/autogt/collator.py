...
class Batch():
    def __init__(self, idx, attn_bias, attn_edge_type, spatial_pos, in_degree, out_degree, x, edge_input, y, lap_pos_enc, svd_pos_enc, pma_att_enc):
        ...
        self.lap_pos_enc, self.svd_pos_enc, self.pma_att_enc = lap_pos_enc, svd_pos_enc, pma_att_enc
    ...
    def to(self, device):
        ...
        self.lap_pos_enc = self.lap_pos_enc.to(device)
        self.svd_pos_enc = self.svd_pos_enc.to(device)
        self.pma_att_enc = self.pma_att_enc.to(device)
        return self
...
def collator(items, max_node=512, multi_hop_max_dist=20, spatial_pos_max=20):
    ...
    items = [(item.idx, item.attn_bias, item.attn_edge_type, item.spatial_pos, item.in_degree,
              ...
              item.lap_pos_enc, item.svd_pos_enc, item.pma_att_enc) for item in items]
    idxs, attn_biases, attn_edge_types, spatial_poses, in_degrees, out_degrees, xs, edge_inputs, ys, lap_pos_encs, svd_pos_encs, pma_att_encs = zip(
        *items)
    ...
    lap_pos_enc = torch.cat([pad_2d_unsqueeze(i, max_node_num) for i in lap_pos_encs])
    svd_pos_enc = torch.cat([pad_2d_unsqueeze(i, max_node_num) for i in svd_pos_encs])
    pma_att_enc = torch.cat(
        [pad_edge_type_unsqueeze(i, max_node_num) for i in pma_att_encs])
    return Batch(
        ...
        svd_pos_enc=svd_pos_enc,
        pma_att_enc=pma_att_enc,
    )
